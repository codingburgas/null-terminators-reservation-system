...
std::string simpleHash(const std::string& input) {
    // djb2 hash - deterministic, simple, good enough for a school project
...
void initAuth() {
    // Seed a default admin account so you can log in immediately
    UserAccount admin;
...
bool renderAuthUI() {
    ...
    if (ImGui::BeginTabBar("AuthTabs")) {
        ...
        if (ImGui::BeginTabItem("Create Account")) {
            ...
            // First / Last name side by side - table so each label sits above its own input
